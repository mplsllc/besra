...
not by ease. The honest frame up front, then the ranked work.*
...
## 1. JS → DOM → layout round-tripping  *(the wall; do this first)*
...
- MacSurf started this: QuickJS (ES2023) replaced Duktape, plus a reconvert-after-
  ...
  work — the QuickJS integration and the DOM-mutation→re-layout bridge — is the seed.
...
- Sub-parts: live DOM node bindings, event dispatch (click/submit/input/change) wired
  ...
  mutation → invalidation → reflow.

## 2. `fetch` / XHR  *(no JS networking = no dynamic content)*
