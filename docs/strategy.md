...
not the "what to build" (that's [roadmap.md](roadmap.md)). For current build status
and the concrete execution plan, see [plan.md](../plan.md) and
[CLAUDE.md](../CLAUDE.md) at the repo root.*

**Update since founding:** the ownership decision below has been made. Besra is a
hard fork (no upstream remote, no periodic rebase plan). The frontend is Qt6, chosen
specifically as the cross-platform layer for the Linux-then-Windows-then-Mac plan,
and Besra is also slated as the default browser for the maintainer's new
XFCE-based Linux distro, which reinforces the "lightweight but capable" positioning
below with a real, guaranteed user base.
...
that stopped evolving."* That is a dramatically better starting line than a blank
repo. You inherit the three years of unglamorous foundation work for free.
