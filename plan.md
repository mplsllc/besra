...
Qt6 is the portability layer — platform-specific code should route through Qt/libcurl,
not bare glibc, so Win/Mac stay cheap later. See `.private/docs/` for the founding
vision/strategy and memory (besra-build-strategy, besra-devtable-progress,
besra-product-roadmap) for the fuller record.
...
* **Step 5:** Vendor the libraries (`libwapcaplet`, `libdom`, etc.) in as real flattened
  source — drop the "independent projects" pretense (CMakeLists.txt per lib are already
