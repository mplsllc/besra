 ...
 * libcss's lexer tokenizes "--foo" as CHAR('-') + IDENT('-foo') in some
 * lexer states even after the CDCOrIdentOrFunctionOrNPD fix (escaped
 * names, edge cases); normalizing up to two leading dashes on each side
 * keeps the pipeline correct for either tokenization. */
...
/* ------------------------------------------------------------------ */
/* Document-global inline-extras custom-property table                */
/* ------------------------------------------------------------------ */
/*
 * Aggregates custom-property declarations harvested from EVERY inline
 * style="..." attribute parsed by the frontend's inline-style creation
 * path. This is the pragmatic V1 of element-scoped custom properties:
 * when an element defines `style="--header-tile: url(...)"` and the
 * property is referenced from author CSS that matches a DIFFERENT
 * element (typically a descendant), libcss's per-sheet
 * custom_properties lists don't see it. The CSS spec answer is
 * inheritance, but our cascade implements custom props as
 * parse-time-captured tokens, not computed style inheritance, so we
 * route around the gap by aggregating all inline custom props into a
 * doc-global table that lookup_var() consults.
 *
 ...
 * Spec-correct behaviour would scope per element.
 *
 * Cleanup: css_inline_extras_clear() must be called on new top-level
 * document navigation so the table doesn't accumulate/bleed across
 * page loads (NOT on every html_content create -- iframes are html
 * content too, and clearing on iframe load would wipe out a still-live
 * parent document's inline custom properties mid-render).
 */
		...
		uint32_t *fb_start, uint32_t *fb_end)
{
	...
	*fb_end = body_end;
	 ...
	 * (which the lexer now emits after the CDCOrIdentOrFunctionOrNPD
	 * fix), or the CSS-2.1 lexer split sequence CHAR('-') +
	 * IDENT('-foo') (still possible in some lexer states). */
...
/* Count/get-sheet hooks implemented in select.c so we don't have to
 ...
 *
 * Also consults the per-element inline_style sheet (when provided).
 * Highest priority (CSS spec puts inline-style declarations above
 * author cascade), so checked last -- found = hit overwrites any
 * earlier match. Lets `background-image: var(--header-tile)` in an
 * author stylesheet resolve against `style="--header-tile: url(...)"`
 * set inline on the matched element. */
		...
		const css_stylesheet *inline_sheet)
{
	...
	/* Doc-global inline-extras table. Aggregates custom properties
	 * from EVERY element's inline style="..." so a parent's
		...
		css_select_state *state)
{
	...
	/* Apply bytecode through the normal cascade dispatch, but
	 * temporarily bump current_specificity to MAX so the var()-
