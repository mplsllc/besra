...
lighter than a Chromium/Gecko browser. See `README.md` and `docs/` for the full founding
vision, strategy, and roadmap, and `plan.md` for the live, ground-truth execution status.

**Cross-platform direction:** Linux first (default browser for the user's new
XFCE-based distro), then Windows, then Mac. Qt6 is the portability layer, which is why
the platform layer should route through Qt/libcurl, not bare glibc: Win/Mac should
become "recompile and package," not "write a new frontend."
...
- `docs/`: the founding strategy, roadmap, and engine-basis documents.
- `plan.md`: the live execution roadmap. **Read this first** for current status; it's
  more current than this file for day-to-day state.
- `.private/`: gitignored. Scratch/debug artifacts (`scratch/`), reference material
  pulled from elsewhere (`reference/`, e.g. the original NetSurf `qt` frontend kept
  for reading, not building against).
...
The single biggest structural change from stock NetSurf: the `gui_table`
function-pointer vtable indirection is gone. Stock NetSurf routes every frontend
operation through a `struct netsurf_table` of function-pointer tables
(`gui_window_table`, `gui_bitmap_table`, etc.), assembled at startup via
`netsurf_register()` into a global `guit`, so the core calls
`guit->window->redraw(...)`. That existed to support many interchangeable frontends
(RISC OS, Amiga, GTK, Windows...). Besra has exactly one frontend, so the indirection
was collapsed to **direct link-time calls**:
...
- **Platform-layer code should target Qt/libcurl, not bare glibc.** NetSurf's original
  ...
  Windows/Mac stay a recompile away rather than a rewrite. See plan.md's "Step 4b"
  note for the specific follow-up owed here.
...
Once the engine/build foundation is solid: ad and tracker blocking (do first, cheap
and on-thesis; `frontends/gtk/res/adblock.css` is prior art), password management
(mine MacSurf's `password-manager` branch for prior art), extensions (gated on JS/DOM
maturity, Duktape to QuickJS swap first), and cloud sync. For sync, lean toward a
trusted third party or bring-your-own-storage with client-side end-to-end encryption,
or a Bitwarden-compatible approach for passwords specifically. Do not roll a sync
backend; it makes the maintainer a custodian of everyone's data, which is the wrong
kind of ownership for a solo-maintained project.
